...
export const followUnFollowUser = async (req, res) => {
  try {
    ...
    if (id === req.user._id) {
      return res.status(404).json({ error: "You con't unfollow/follow" });
    }
...
export const getSuggestedUsers = async (req, res) => {
  try {
    ...
    console.log(`Error in getsSuggestedUsers controller : ${error}`);
    res.status(500).json({ error: "Internal Server Error" });
  }
};
  ...
  try {
    ...
    if (currentPassword && newPassword) {
      ...
      if (newPassword.length < 6) {
        return (
          res, status(404).json({ error: "password must have atleast 6 char" })
        );
      }
